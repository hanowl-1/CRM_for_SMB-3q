          ...
          for (const variableMapping of step.action.personalization.variableMappings) {
            ...
            try {
              ...
              if (savedMappings && savedMappings.length > 0) {
                console.log(`📋 ${savedMappings.length}개의 저장된 변수 매핑 발견`);
                
                for (const mapping of savedMappings) {
                  if (mapping.source_type === 'query' && mapping.source_field && mapping.selected_column) {
                    console.log(`🔍 실행 시 쿼리 변수 처리:`, { 
                      variableName: mapping.variable_name, 
                      sourceField: mapping.source_field, 
                      selectedColumn: mapping.selected_column, 
                    ...
                    const variableQueryResult = await fetch('http://localhost:3000/api/mysql/query', {
                      ...
                      body: JSON.stringify({ 
                        query: mapping.source_field, // 원본 쿼리 그대로 실행
                    ...
                    if (variableQueryResult.ok) {
                      ...
                      if (variableData.success && variableData.data && variableData.data.length > 0) {
                        ...
                        if (mapping.key_column) {
                          if (mapping.key_column.includes('.')) {
                            // 'a.id' 같은 경우 -> 'id'로 변환
                            const simpleColumn = mapping.key_column.split('.').pop();
                            targetMappingValue = rawData[simpleColumn] || rawData.id || target.id;
                          } else {
                            targetMappingValue = rawData[mapping.key_column] || rawData.id || target.id;
                          }
                        }
                        ...
                        // 3. 메모리에서 매칭 수행 (AA열 ↔ BB열 매칭)
                        const mappingColumn = mapping.key_column || 'id'; // 매칭에 사용할 변수 데이터의 컬럼
                        ...
                        const matchedRow = variableData.data.find(row => {
                          ...
                          let variableMappingValue;
                          
                          // 1) 설정된 keyColumn 사용 시도
                          if (row[mappingColumn] !== undefined) {
                            variableMappingValue = row[mappingColumn];
                          }
                          // 2) adId 컬럼 사용 시도 (리뷰 데이터의 경우)
                          else if (row['adId'] !== undefined) {
                            variableMappingValue = row['adId'];
                          }
                          // 3) id 컬럼 사용 시도
                          else if (row['id'] !== undefined) {
                            variableMappingValue = row['id'];
                          }
                          // 4) 첫 번째 컬럼 사용
                          else {
                            variableMappingValue = row[availableColumns[0]];
                          }
                        ...
                        value = mapping.default_value || '--';
                        console.log(`⚠️ 실행 시 변수 쿼리 결과 데이터 없음:`, { variableName: mapping.variable_name });
                      }
                    } else {
                      value = mapping.default_value || '--';
                      console.log(`❌ 실행 시 변수 쿼리 실행 실패:`, { variableName: mapping.variable_name, status: variableQueryResult.status });
                    }
                  } else if (mapping.source_type === 'field' && mapping.source_field) {
                    // 필드 매핑인 경우
                    const rawData = target.rawData || target;
                    const fieldValue = rawData[mapping.source_field];
              ...
              console.error(`❌ 실행 시 변수 매핑 오류:`, mappingError);
            }
